...
handle_messages (struct eXosip_t *excontext, struct _tcp_stream *sockinfo)
{
  ...
  while (buflen > 0 && (end_headers = buffer_find (buf, buflen, END_HEADERS_STR)) != NULL) {
    ...
    *end_headers = END_HEADERS_STR[0];
    ...
    if (msglen > (int)buflen) {
      /* nope */
      return consumed;
    }
    /* yep; handle the message */
    _eXosip_handle_incoming_message (excontext, buf, msglen, sockinfo->socket, sockinfo->remote_ip, sockinfo->remote_port, sockinfo->natted_ip, &sockinfo->natted_port,sockinfo->socket);
    consumed += msglen;
    buflen -= msglen;
    buf += msglen;
  }
...
_tcp_tl_recv (struct eXosip_t *excontext, struct _tcp_stream *sockinfo)
{
  ...
  //  _tcp_tl_close_sockinfo (sockinfo);
    ...
    if (consumed == 0) {
      ...
      if ((int)sockinfo->buflen > consumed) {
...
_tcp_tl_is_connected (int sock)
{
  ...
  _tcp_tl_connect_socket (struct eXosip_t *excontext, char *host, int port)
  {
	...
	static int tcp_tl_check_socket(struct eXosip_t *excontext,int sock)
	{
		int pos;
		struct eXtltcp *reserved = (struct eXtltcp *) excontext->eXtltcp_reserved;
		if (reserved == NULL) {
		...
		if(reserved->tcp_socket == sock)
		{
			return OSIP_SUCCESS;
		}
		for (pos = 0; pos < EXOSIP_MAX_SOCKETS; pos++) {
			if (reserved->socket_tab[pos].socket > 0 && reserved->socket_tab[pos].socket == sock) {
				return OSIP_SUCCESS;
			}
		}
