...
eXosip_quit (struct eXosip_t *excontext)
{
  ...
  int eXosip_Socket_IsAlive(struct eXosip_t *excontext,int sock)
  {
	  if (excontext == NULL) {
		  return OSIP_WRONG_STATE;
	  }
	  if(excontext->eXtl_transport.tl_check_socket == NULL)
	  {
		  return OSIP_SUCCESS;
	  }

	  return excontext->eXtl_transport.tl_check_socket(excontext,sock);
  }
...
_eXosip_thread (void *arg)
{
  ...
  while (excontext->j_stop_ua == 0) {
    ...
    if (i == -2000)
      osip_thread_exit ();
#ifndef WIN32 //½â¾öexosip cpu¸ßÈ±ÏÝ
	if(times ++ >= 100){
		usleep(100000);
		times = 0;
	}
