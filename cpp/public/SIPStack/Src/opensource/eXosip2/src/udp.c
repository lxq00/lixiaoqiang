...
int
_eXosip_handle_incoming_message (struct eXosip_t *excontext, char *buf, size_t length, int socket, char *host, int port, char *received_host, int *rport_port,int sock)
{
  int i;
  osip_event_t *se;
  int tmp;
  ...
  se->sip->sock.port = port;
  se->sip->sock.addr = 0;
  se->sip->sock.sock = sock;
  if(host != NULL)
  {
	  se->sip->sock.addr = inet_addr(host);
  }
