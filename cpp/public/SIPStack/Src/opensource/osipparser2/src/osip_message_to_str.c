...
_osip_message_to_str (osip_message_t * sip, char **dest, size_t * message_length, int sipfrag)
{
  ...
  *dest = message;
  ...
  /* we NOW have the length of bodies: */
  {
	...
	//modiy by lxq,changed connent-len buffer
	if (strlen(tmp2) < 5)
	{
		strncpy(content_length_to_modify, tmp2, strlen(tmp2));
		memmove(content_length_to_modify + strlen(tmp2), content_length_to_modify + 5, message - content_length_to_modify + 5);

		total_length -= (5 - strlen(tmp2));
	}
  }
