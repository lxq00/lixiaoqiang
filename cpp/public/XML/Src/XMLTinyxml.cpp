...
void parseTiXmlElementAndAddChild(TiXmlNode* pNode,XM_XML::Child& child)
{
	if(pNode == NULL)
	{
		return;
	...
	while(pChildNode != NULL)
	{
		...
		{
			XM_XML::Child& subchild = child.addChild(pChildNode->Value());
...
void buildTiXmlElementFormChild(XM_XML::Child& child,TiXmlElement* pElement,XM_XML::Encoding old,XM_XML::Encoding encode)
{
	...
	XM_XML::Attribute atti = child.firstAttribute();
	while(!atti.isEmpty())
	{
	...
	XM_XML::Child subchild = child.firstChild();
...
void findVersionAndEncoding(const std::string& xml,std::string& ver,XM_XML::Encoding& encode)
{
	...
	do
	{
		...
		{
			encode = XM_XML::Encoding_GBK;
		}
		else if(strcasecmp(codingbuf,"utf-8") == 0)
		{
			encode = XM_XML::Encoding_UTF8;
		}
		break;
	}while(1);
}
...
{
	if(old == XM_XML::Encoding_Unknown)
	{
		old = XM_XML::Encoding_GBK;
	}
	if(encode == XM_XML::Encoding_Unknown)
	{
		encode = old;
	}
