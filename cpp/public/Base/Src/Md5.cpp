...
//
//	Description:
//	$Id: Md5.cpp 11 2013-01-22 08:42:03Z jiangwei $
//
...
static void
MD5_Update(MD5_CTX *context, unsigned char *input, unsigned int inputLen)
{
	unsigned int i;
	unsigned int index;
...
MD5_Final(unsigned char digest[16], MD5_CTX *context)
{
    ...
    (void)MD5_Update((MD5_CTX*)context, (unsigned char*)PADDING, (unsigned int)padLen);

    /* Append length (before padding) */
    (void)MD5_Update((MD5_CTX*)context, (unsigned char*)bits, (unsigned int)8);
...
Md5::Md5()
{
	internal = new(std::nothrow) Md5Internal;
	memset(internal, 0, sizeof(Md5Internal));
	MD5_Init(&(internal->md5_ctx));
}
...
void Md5::init()
{
	internal->is_final = false;
	MD5_Init(&(internal->md5_ctx));
	memset(internal->digest, 0, sizeof(internal->digest));
}

void Md5::update(uint8_t const* data, size_t size)
{
	if (data != NULL && size > 0)
	{
		...
		MD5_Update(&(internal->md5_ctx), (unsigned char*)data, (unsigned int)size);
	}
}
