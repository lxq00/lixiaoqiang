...
std::string getFileNameByIndex(const std::string& filename,int index)
{
	tostringstream source_oss;

	const char* fileapend = strrchr(filename.c_str(),'.');

	source_oss << std::string(fileapend == NULL ? filename : std::string(filename.c_str(),fileapend - filename.c_str()));

	source_oss << LOG4CPLUS_TEXT(".") << index;

	if(fileapend != NULL)
	{
		source_oss << fileapend;
	}
...
rolloverFiles(const tstring& filename, unsigned int maxBackupIndex)
{
    ...
    buffer << getFileNameByIndex(filename,maxBackupIndex);
    ...
    for (int i = maxBackupIndex - 1; i >= 1; --i)
    {
        ...
        source_oss << getFileNameByIndex(filename,i);
        target_oss << getFileNameByIndex(filename,(i+1));
...
DailyRollingFileAppender::rollover(bool alreadyLocked)
{
    ...
    backup_target_oss << getFileNameByIndex(scheduledFilename,1);
...
DailyRollingFileAppender::getFilename(const Time& t) const
{
	...
	const char* fileappend = strrchr(filename.c_str(),'.');

    tstring result (fileappend == NULL ? filename:std::string(filename.c_str(),fileappend - filename.c_str()));
