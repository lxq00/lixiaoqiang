...
bool SocketConnection::setSocketOpt(int level, int optname, const void *optval, int optlen)
{
	boost::shared_ptr<TCPSocketObject>	sockinternal;
	{
		Guard locker(mutex);
	...
	return sockinternal->setSocketOpt(level, optname, optval, optlen);
}
bool SocketConnection::getSocketOpt(int level, int optname, void *optval, int *optlen) const
{
	boost::shared_ptr<TCPSocketObject>	sockinternal;
	{
		//Guard locker(mutex);
	...
	return sockinternal->getSocketOpt(level, optname, optval, optlen);
}
...
bool SocketConnection::async_recv(char *buf , uint32_t len,const ReceivedCallback& received)
{
	...
	if(sockinternal == NULL || buf == NULL || len == 0 || !received)
	{
		return false;
	}
...
bool SocketConnection::async_recv(const ReceivedCallback& received, int maxlen)
{
	boost::shared_ptr<TCPSocketObject>	sockinternal;
	{
		Guard locker(mutex);
	...
	if (sockinternal == NULL || maxlen == 0 || !received)
	{
		return false;
	}

	boost::shared_ptr<RecvInternal> recvptr = boost::make_shared<TCPRecvInternal>((Socket*)this, maxlen, received);
...
bool SocketConnection::async_send(const char * buf, uint32_t len,const SendedCallback& sended)
{
	...
	if(sockinternal == NULL || buf == NULL || len == 0 || !sended)
	{
		return false;
	}
