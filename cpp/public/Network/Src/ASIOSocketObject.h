...
{
public:
	RecvInternal(Socket* _sock,char* addr,uint32_t len):recvBuffer((char*)addr),recvBufferLen(len),sock((Socket*)_sock){}
...
{
public:
	TCPRecvInternal(Socket* _sock,char* addr,uint32_t len,const Socket::ReceivedCallback& recved)
	:RecvInternal(_sock,addr,len),recvCallback(recved){}
	~TCPRecvInternal(){}
	virtual void setRecvResult(int recvLen)
	{
...
{
public:
	udpRecvInternal(Socket* _sock,char* addr,uint32_t len,const Socket::RecvFromCallback& recved)
	:RecvInternal(_sock,addr,len),recvCallback(recved){}
	~udpRecvInternal(){}
	virtual void setRecvResult(int recvLen)
	{
