...
IOWorker::IOWorkerInternal::IOWorkerInternal(uint32_t num):poolQuit(false)
{
	...
	{
		Thread* thread = ThreadEx::creatThreadEx("IOServerRun",ThreadExProc(&IOWorker::IOWorkerInternal::threadRunProc,this),NULL,Thread::priorTop,Thread::policyRealtime);
		thread->createThread();

		boost::shared_ptr<Thread> threadptr(thread);
		threadPool.push_back(threadptr);
	}
}
...
void IOWorker::IOWorkerInternal::threadRunProc(void* param)
{
	while(!poolQuit)
	{
		try
