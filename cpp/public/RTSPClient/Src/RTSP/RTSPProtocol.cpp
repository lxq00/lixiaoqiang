...
RTSPProtocol::RTSPProtocol(const std::string& rtspUrl)
	:m_videoSdp(0),m_audioSdp(2), m_nSpsLen(0), m_nPpsLen(0), m_nFrameRate(25)
{
...
void RTSPProtocol::doDispatcherCMD(const RTSP_RESPONSE& repose)
{
	...
	if (errorcode == 401 && node.cmdbuilder->cmdType == RTSPCmd_DESCRIBE)
	{
	...
	{
		if (node.cmdbuilder->cmdType == RTSPCmd_DESCRIBE)
		{
			//分析sdp, 得到音视频信息
