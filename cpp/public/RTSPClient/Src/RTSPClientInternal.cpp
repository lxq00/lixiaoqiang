...
{
	m_videoUDPRecvBuffer = m_videoCtrlRecvBuffer = m_audioUDPRecvBuffer = m_audioUDPCtrlRecvBuffer = NULL;
	m_lPrevVideoTs = -1;
	m_lVideoTsDiff = 0;
	m_lCurrentVideoTs = 0;
	m_nFrameIndex = 0;
...
bool RTSPClient::RTSPClientInternal::start(const shared_ptr<IOWorker>& worker, uint32_t timeout, uint32_t startUdpport)
{
	...
	if (RTSPProtocol::m_nRecvType == RTP_RECV_TYPE_UDP)
	{
		...
		m_videoUDPRecvBuffer = new char[MAXRECVBUFFERSIZE];
		m_videoCtrlRecvBuffer = new char[MAXCTRLBUFFERSIZE];
		m_audioUDPRecvBuffer = new char[MAXRECVBUFFERSIZE];
		m_audioUDPCtrlRecvBuffer = new char[MAXCTRLBUFFERSIZE];

		m_videoUDPRecv = make_shared<UDP>(*RTSPProtocol::m_ioworker.get());
		...
		m_videoCtrlUDPRecv = make_shared<UDP>(*RTSPProtocol::m_ioworker.get());
		...
		m_audioUDPRecv = make_shared<UDP>(*RTSPProtocol::m_ioworker.get());
		...
		m_audioCtrlUDPRecv = make_shared<UDP>(*RTSPProtocol::m_ioworker.get());
...
bool RTSPClient::RTSPClientInternal::stop()
{
	...
	SAFE_DELETEARRAY(m_videoUDPRecvBuffer);
	SAFE_DELETEARRAY(m_videoCtrlRecvBuffer);
	SAFE_DELETEARRAY(m_audioUDPRecvBuffer);
	SAFE_DELETEARRAY(m_audioUDPCtrlRecvBuffer);
