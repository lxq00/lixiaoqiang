...
bool RTSPClient::RTSPClientInternal::start(const shared_ptr<IOWorker>& worker, uint32_t timeout, uint32_t startUdpport)
{
	...
	m_poolTimerProc = new Timer("RTSPClient::RTSPClientInternal");
...
int RTSPClient::RTSPClientInternal::creatRTPAnalyzer()
{
	...
	{
		m_pRtpAnalyzer = new RtpJpegAnalyzer(m_stMediaInfo.stStreamVideo.nPayLoad, m_stMediaInfo.stStreamAudio.nPayLoad);
	}
	else
	{
