...
public:
	...
	virtual std::string build(const URI& uri) = 0;
	virtual bool parse(XMLN * p_xml) = 0;
protected:
	std::string  buildHeader(const URI& uri)
	{
		std::string noneBase64, passwdBase64, createTime;

		buildUserAuthenInfo(noneBase64, passwdBase64, createTime, uri.authen.Password);

		char buffer[1024] = { 0 };

		snprintf_x(buffer, 1023, onvif_xml_header, uri.authen.Username.c_str(), passwdBase64.c_str(), noneBase64.c_str(), createTime.c_str());

		return buffer;
	}

	std::string buildCreateHeader(const URI& uri)
	{
		std::string noneBase64, passwdBase64, createTime;
		...
		snprintf_x(buffer, 1023, onvif_xml_headerCreate,
			guidstr.c_str(), uri.authen.Username.c_str(), passwdBase64.c_str(), noneBase64.c_str(), createTime.c_str(), uri.getHost().c_str(), uri.getPath().c_str());
