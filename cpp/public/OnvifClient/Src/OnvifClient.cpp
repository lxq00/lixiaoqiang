...
struct OnvifClient::OnvifClientInternal
{
	...
	{
		shared_ptr<HTTPClient> client = make_shared<HTTPClient>(worker, url);
		...
		URI::Value contenttypeval = response->getHeader("Content-Type");
		if (strstr(String::tolower(contenttypeval.readString()).c_str(), ONVIFECONENTTYPE) == 0)
		{
			return false;
		}

		std::string httpbody = response->buf.read();
...
OnvifClient::OnvifClient(const URI& url, const shared_ptr<IOWorker>& worker, const std::string& useragent)
{
...
{
	shared_ptr<CmdGetStreamUri> cmd = make_shared<CmdGetStreamUri>(streamtoken);
...
{
	shared_ptr<CmdGetSnapUri> cmd = make_shared<CmdGetSnapUri>(snaptoken);
