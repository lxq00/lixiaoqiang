...
Write_Range::Write_Range(const shared_ptr<XM_Excel::Sheet>& _worksheet, uint32_t _startRowNum, uint32_t _startColNum, uint32_t _stopRowNum, uint32_t _stopColNum, xlslib_core::range* __range)
...
//--------------字体相关		
bool Write_Range::font(const shared_ptr<XM_Excel::Font>& _font)
{
...
//--------------颜色相关,填充颜色
bool Write_Range::fill(const XM_Excel::Color& _color)
{ 
	//this de father
	Write_Sheet* sheet = (Write_Sheet*)worksheet.get();
	//sheet de father
...
bool Write_Range::fill(const shared_ptr<XM_Excel::Color>& _color)
{ 
	fill(*_color.get());
	return true;
}
...
//--------------显示格式
bool Write_Range::format(const XM_Excel::Format& _fmt)
{ 
...
bool Write_Range::format(const shared_ptr<XM_Excel::Format>& _fmt)
{ 
	format(*_fmt.get());
	return true;
}
...
//--------------单元格边框
bool Write_Range::side(const XM_Excel::Side& _sd)
{ 
	//this de father
	Write_Sheet* sheet = (Write_Sheet*)worksheet.get();
	//sheet de father
...
bool Write_Range::side(const shared_ptr<XM_Excel::Side>& _sd)
{ 
	side(*_sd.get());
	return true;
}
