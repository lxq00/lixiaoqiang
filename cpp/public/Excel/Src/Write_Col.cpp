...
//设置数据，内容
shared_ptr<WorkBook::Cell> Write_Col::setData(uint32_t rowNum, const shared_ptr<WorkBook::Value>& val)
{
	if (val == NULL)
...
shared_ptr<WorkBook::Cell> Write_Col::setData(uint32_t rowNum, const WorkBook::Value& val)
{
	if (worksheet == NULL)
		return shared_ptr<WorkBook::Cell>();
