...
///设置数据，内容
shared_ptr<WorkBook::Cell> Write_Sheet::setData(uint32_t rowNum, uint32_t colNum, const shared_ptr<WorkBook::Value>& val)
{
...
shared_ptr<WorkBook::Cell> Write_Sheet::setData(uint32_t rowNum, uint32_t colNum, const WorkBook::Value& val)
{
	//cell_t* m_cell;
	if (writesheet == NULL)
	{
	...
	cell_t* m_cell = NULL;
	if (val.type() == WorkBook::Value::Type_Bool)
	{
		m_cell = writesheet->boolean(rowNum, colNum, val.readBool());
	}
	else if (val.type() == WorkBook::Value::Type_Int32)
	{
		m_cell = writesheet->number(rowNum, colNum, val.readUint32());
	}
	else if (val.type() == WorkBook::Value::Type_Double)
	{
