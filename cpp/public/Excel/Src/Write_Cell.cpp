...
bool Write_Cell::font(const shared_ptr<XM_Excel::Font>& font)
{
...
//--------------填充颜色
bool Write_Cell::fill(const XM_Excel::Color& _color)
{
	if (m_cell == NULL)
	{
		return false;
...
bool Write_Cell::fill(const shared_ptr<XM_Excel::Color>& _color)
{ 
	if (_color == NULL)
	{
		return false;
...
//--------------显示格式
bool Write_Cell::format(const XM_Excel::Format& _fmt)
{ 
	//this de father
	Write_Sheet* sheet = (Write_Sheet*)worksheet.get();
	//sheet de father
...
bool Write_Cell::format(const shared_ptr<XM_Excel::Format>& _fmt)
{ 
	if (_fmt == NULL)
	{
		return false;
...
//--------------单元格边框
bool Write_Cell::side(const XM_Excel::Side& _sd)
{ 
	if (m_cell == NULL)
	{
		return false;
...
bool Write_Cell::side(const shared_ptr<XM_Excel::Side>& _sd)
{	
	if (_sd == NULL)
	{
		return false;
