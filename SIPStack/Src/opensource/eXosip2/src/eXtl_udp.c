...
static size_t udp_message_max_length = 64 * 1024; ;// SIP_MESSAGE_MAX_LENGTH;
#else
static int udp_message_max_length = 64 * 1024; ;// SIP_MESSAGE_MAX_LENGTH;
...
udp_tl_read_message (struct eXosip_t *excontext, fd_set * osip_fdset, fd_set * osip_wrset)
{
  ...
  if (FD_ISSET (reserved->udp_socket, osip_fdset)) {
    ...
    if (i > 32) {
      ...
      _eXosip_handle_incoming_message (excontext, reserved->buf, i, reserved->udp_socket, src6host, recvport, NULL, NULL,reserved->udp_socket);
  ...
  if (reserved->udp_socket_oc > 0 && FD_ISSET (reserved->udp_socket_oc, osip_fdset)) {
    ...
    if (i > 32) {
      ...
      _eXosip_handle_incoming_message (excontext, reserved->buf, i, reserved->udp_socket_oc, src6host, recvport, NULL, NULL,reserved->udp_socket);
    }
    else if (i < 0) {
#ifdef _WIN32_WCE
      int my_errno = 0;
