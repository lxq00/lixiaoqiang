...
std::string RTSPUrlInfo::buildWWWAuthen(const std::string& cmd, const std::string& username, const std::string& password)
{
	std::string wwwauthen = std::string("Digest ") + "realm=\"" + Value(Time::getCurrentMilliSecond()).readString() + Value(cmd.c_str()).readString()
		+ "\",nonce=\"" + Value(username.c_str()).readString() + Value(password.c_str()).readString() + "\"";
