...
public:
	HTTPProtocolBuilder(const shared_ptr<OBJECT>& _obj,const shared_ptr<Socket>& _sock, const std::string& _useragent)
...
private:
	void checkSendCallback(const weak_ptr<Socket>&, const char* tmp, int len)
	{
		...
		if(sendlen > 0)
		{
			...
			socket->async_send(sendbuffercache, sendlen, Socket::SendedCallback(&HTTPProtocolBuilder::checkSendCallback, this));
	...
	int buildHeader(char* buffer,shared_ptr<OBJECT>& object, shared_ptr<Socket>& socket)
	{
		...
		else if(strcasecmp(object->headers()[CONNECTION].readString().c_str(),CONNECTION_Upgrade) != 0)
		{
